...
public class BasePage {
	...
	public static CarBase careBase;
	
	public BasePage(Page page) {

		this.page = page;
		careBase = new CarBase(page);
		try {
	...
	public void mouseHover(String locatorKey) {
		...
		try {
			...
			ExtentListeners.getExtent().info("Hover over an Element : " + locatorKey);
		} catch (Throwable t) {
			
			ExtentListeners.getExtent()
					.fail("Error while Hovering on an Element : " + locatorKey + " error message is :" + t.getMessage());
			Assert.fail(t.getMessage());
		}
	}
